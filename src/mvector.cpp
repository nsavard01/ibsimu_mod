...
Vector::Vector( const Vector::VectorLA &vecla ) 
{
	...
	cblas_daxpy( _n, vecla._refs[a]._coef, vecla._refs[a]._vec->_val, 1, _val, 1 );
#else
    for( int i = 0; i < _n; i++ ) {
...
Vector &Vector::operator=( const Vector::VectorLA &vecla ) 
{
	...
	cblas_daxpy( _n, vecla._refs[a]._coef, vecla._refs[a]._vec->_val, 1, res, 1 );
#else
    for( int i = 0; i < _n; i++ ) {
...
double dot_prod( const Vector &vec1, const Vector &vec2 )
{
	...
	throw( ErrorDim( ERROR_LOCATION ) );
#ifdef USE_MKL
    return( cblas_ddot( vec1._n, vec1._val, 1, vec2._val, 1 ) );
#else
    double res = 0.0;
...
{
#ifdef USE_MKL
    return( cblas_dasum( vec._n, vec._val, 1 ) );
#else
    double res = 0.0;
    for( int i = 0; i < vec._n; i++ )
	res += fabs( vec._val[i] );
...
{
#ifdef USE_MKL
    return( cblas_dnrm2( vec._n, vec._val, 1 ) );
#else
    double res = 0.0;
    for( int i = 0; i < vec._n; i++ )
...
{
    // ddot(v,v) == sum(v_i^2); MKL parallelises this automatically.
#ifdef USE_MKL
    return( cblas_ddot( vec._n, vec._val, 1, vec._val, 1 ) );
#else
    double res = 0.0;
    for( int i = 0; i < vec._n; i++ )
	res += vec._val[i]*vec._val[i];
